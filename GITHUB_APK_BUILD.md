...
## Python 3.14 / pyjnius 오류 수정

이 버전은 Android 빌드용 Python을 `python3==3.11.14`와 `hostpython3==3.11.14`로 고정합니다.
GitHub Actions 로그에서 `--python-version 3.14.x`가 보이던 문제를 피하기 위한 설정입니다.
또한 매 빌드 전에 `.buildozer`와 `bin`을 삭제하여 이전 Python 빌드 결과가 섞이지 않게 합니다.
